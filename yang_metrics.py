...
def calculaterRpar(y_pred,_protected_group,_user_N,_pro_N):
    ...
    """
    groups = [0 if x in _protected_group else 1 for x in y_pred ]
    e0,e1 = rank_parity(y_pred,groups)
    e = 1.0 - abs(e0 - e1) # reverse to match semantics of other metrics - large value -> more fair
    if e == 0.0:
        e = 0.001
    return e 
...
def calculaterRpar0(y_pred,_protected_group,_user_N,_pro_N):
    ...
    """
    groups0 = [0 if x in _protected_group else 1 for x in y_pred ]
    e0,e1 = rank_parity(y_pred,groups0)
    e =  1.0 - e0 # reverse to match semantics of other metrics - large value -> more fair
    if e == 0.0:
        e = 0.001
    return e 
...
def calculaterRpar1(y_pred,_protected_group,_user_N,_pro_N):
    """
        Calculate the Rank Parity metric for a single group
    """
    groups1 = [0 if x in _protected_group else 1 for x in y_pred ]
    e0,e1 = rank_parity(y_pred,groups1)
    e = 1.0 - e1 # reverse to match semantics of other metrics - large value -> more fair
    if e == 0.0:
        e = 0.001
    return e 
